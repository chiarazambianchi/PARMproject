...
```{r}
bed1<-import("metadata_fold0.bed.gz",header = TRUE, sep = "\t" )
bed2<-import("metadata_fold1.bed.gz", header = TRUE, sep = "\t")
bed3<-import("metadata_fold2.bed.gz", header = TRUE, sep = "\t")
bed4<-import("metadata_fold3.bed.gz", header = TRUE, sep = "\t")
bed5<-import("metadata_test.bed.gz", header = TRUE, sep = "\t")
all_bed<-c(bed1,bed2,bed3,bed4,bed5)
all_bed <- sort(all_bed)
merged_bed<-reduce(all_bed)
head(merged_bed)
```
...
# Extract columns matching e.g., "X1B1", "X1B2"
activity_cols <- grep("^X\\d+.*B\\d$", colnames(filtered_df), value = TRUE)

# Pivot to long format and extract common condition label (e.g., "X1")
long_df <- filtered_df %>%
  select(gene_transcript, all_of(activity_cols)) %>%
  pivot_longer(
    cols = all_of(activity_cols),
  ...
  mutate(group = gsub("B\\d+", "", condition))  # Remove B1/B2 to get "X1", "X2", etc.

# Get unique gene_transcripts
promoters <- unique(long_df$gene_transcript)

# Plot violin plots with grouped conditions
for (p in promoters) {
  p_df <- filter(long_df, gene_transcript == p)
  
  plot <- ggplot(p_df, aes(x = group, y = activity)) +
    geom_violin(trim = FALSE, fill = "lightgreen", alpha = 0.6) +
    geom_jitter(width = 0.2, size = 1.5, color = "darkblue", alpha = 0.6) +
    theme_classic() +
    labs(
      title = paste("Activity for", p),
      x = "Condition (B1+B2 grouped)",
      y = "Activity"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
long_df <- filtered_df %>%
  ...
  mutate(
    replicate_id = str_extract(sample, "^X\\d+.*(?=B[12]$)"),  # more robust
    condition = case_when(
      str_detect(sample, "dmso") ~ "dmso",
      str_detect(sample, "gef") ~ "gef",
      str_detect(sample, "sch77") ~ "sch77",
      TRUE ~ "other"
    )
  )

# Step 3: average B1 and B2 per replicate
avg_df <- long_df %>%
  group_by(gene_transcript, replicate_id, condition) %>%
  summarise(avg_activity = mean(activity, na.rm = TRUE), .groups = "drop")

# Step 4: plot
for (p in unique(avg_df$gene_transcript)) {
  p_df <- filter(avg_df, gene_transcript == p)
  
  plot <- ggplot(p_df, aes(x = condition, y = avg_activity, fill = condition)) +
    geom_violin(trim = FALSE, alpha = 0.4, color = NA) +
    geom_jitter(width = 0.15, size = 2, color = "steelblue", alpha = 0.7) +
    theme_classic() +
    labs(
      title = paste("Average Activity (B1 & B2 merged):", p),
      x = "Condition",
      y = "Mean Activity"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
## Measured vs predicted activity test
...
```{r}
promoters_testset<- unique (df6$gene_transcript)
```
...
ggpairs(all_df_filtered, columns= c(6:11))
```
